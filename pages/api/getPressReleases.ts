...
export default async function handler(
  ...
  // Get Tags
  const tagResp = await cmsRepo.getPressReleaseTags(999999, siteId);
  ...
  const currentTag = tags?.replace(/PR__/g, "")?.split(",");
  ...
  const clientTagList: string[] = currentTag?.map(
    (tag: string): string => tagWithKeys?.[tag] || "__press_Release"
  );
